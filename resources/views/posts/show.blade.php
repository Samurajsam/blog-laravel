    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                        Autor: {{ $post->user->name }} | Added: {{ $post->created_at->format('d.m.Y H:i') }}
                    </p>
                    ...
                    <div class="mt-6 flex gap-4">
                        <a href="{{ route('posts.index') }}" class="text-blue-500 hover:text-blue-700">← Back to list</a>
                        @can('update', $post)
                            <a href="{{ route('posts.edit', $post) }}" class="text-yellow-500 hover:text-yellow-700">Edit</a>
                        @endcan
                        @can('delete', $post)
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h4 class="text-xl font-bold mb-4">Comments ({{ $post->comments->count() }})</h4>
                    ...
                    @auth
                        <form action="{{ route('comments.store') }}" method="POST" class="mb-6">
                            ...
                            <div class="mb-4">
                                <label for="content" class="block text-sm font-medium mb-2">Add comment</label>
                                <textarea 
                                    name="content" 
                                    id="content" 
                                    rows="3" 
                    ...
                    @else
                        <div class="mb-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                            <p class="text-blue-800 dark:text-blue-200">
                                ...
                                <strong>Comments only for logged in users.</strong>
                            </p>
                            <p class="text-blue-700 dark:text-blue-300 mt-2">
                                <a href="{{ route('login') }}" class="font-semibold hover:underline">Log in</a> or 
                                <a href="{{ route('register') }}" class="font-semibold hover:underline">register</a>, to add a comment.
                            </p>
                        </div>
                    @endauth
                    ...
                    @empty
                        <p class="text-gray-600 dark:text-gray-400 mt-4">No comments yet. Be the first!</p>
                    @endforelse
                </div>
            </div>
        </div>
